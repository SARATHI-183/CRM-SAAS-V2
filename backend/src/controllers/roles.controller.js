...
//     if (!name || !role_level) return res.status(400).json({ message: "Name & role_level required" });

//     const id = uuidv4();
//     const role = await knex('master.roles').insert({ id, name, role_level, description }).returning("*");
...
//     return res.status(201).json({ message: "Role created", role: role[0] });
//   } catch (err) {
...
//   try {
//     const { role_id, permission_ids } = req.body; // array of permission UUIDs
...
//   try {
//     const { user_id, role_ids } = req.body; // array of role UUIDs
//     if (!user_id || !role_ids?.length) return res.status(400).json({ message: "user_id and role_ids required" });

//     const inserts = role_ids.map(rid => ({ user_id, role_id: rid }));
