...
export async function assignPermissions(roleId, permissionIds, tenantDb, userId, tenantId) {
  ...
  await tenantDb("role_permissions")
    .insert(inserts)
...
/**
 * ------------------------
 * USER ROLES + USER PERMISSIONS (services for middleware)
 * ------------------------
 */

// ✔️ Fetch only the user’s roles (for requireRole middleware)
export async function getUserRoles(tenantDb, userId) {
  return tenantDb("user_roles as ur")
    .join("roles as r", "ur.role_id", "r.id")
    .where("ur.user_id", userId)
    .select("r.id", "r.name");
}

// ✔️ Fetch permissions across all user roles
export async function getUserPermissions(tenantDb, userId) {
  const roleIds = await tenantDb("user_roles").where({ user_id: userId }).pluck("role_id");
  ...
  return tenantDb("role_permissions as rp")
    ...
    .pluck("p.key"); // returns ["leads.view", "deals.create"]
}
 ...
 */
export async function getUserRolesPermissions(userId, tenantDb) {
  const roles = await getUserRoles(tenantDb, userId);
  const permissions = await getUserPermissions(tenantDb, userId);
  return { roles, permissions };
}
