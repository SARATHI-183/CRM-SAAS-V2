...
// src/db/tenantManager.js
import { knex } from './knex.js';
import { createTenantSchema } from './tenantSchema.js';
import { LRUCache } from 'lru-cache';
let redisClient = null;
...
const cache = new LRUCache({ max: 5000, ttl: 600_000 });
...
/**
 * Creates a new tenant with all tables (calls tenantSchema internally)
 */
export async function createTenant({ schemaName, industryType }) {
  return knex.transaction(async (trx) => {
    await createTenantSchema(schemaName, industryType, trx);
    // Optionally insert default users, modules, etc. here
  });
}
